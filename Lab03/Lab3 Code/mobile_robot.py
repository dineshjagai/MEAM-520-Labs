...
class MobileRobot():
    ...
    def __init__(self, x=np.array([0, 0, 0]), world=None):
        '''
    ...
    def get_state(self):
        ...
        if self.world is not None:
            xy = self.cur_state[0][0:2]
            for o in self.world.InflatedBlocks:
                if o[0] < xy[0] < o[1] and o[2] < xy[1] < o[3]:
                    print("Failure: robot crashed at ", np.round(xy, 2))
                    raise ValueError
    ...
    def set_world(self, world):
        '''
        set world the robot lives in with an occupancy map object
        
        Parameters:
            world :
                OccupancyMap object defining obstacles around robot
        '''
        self.world = world
